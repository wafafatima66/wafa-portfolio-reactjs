      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
            <h3
              className={`font-bold text-gray-900 group-hover:text-blue-600 transition-colors duration-300 ${isExam ? "text-2xl" : "text-xl"}`}
            >
              {item.title}
            </h3>
            {item.result && (
              <span
                className={`px-2 py-0.5 rounded font-bold uppercase tracking-tighter ${isExam ? "text-sm bg-blue-600 text-white" : "text-[10px] bg-blue-50 text-blue-700"}`}
              >
                {item.result}
              </span>
            )}
            ...
            <div className="mt-4">
              ...
              <div className="flex flex-wrap gap-2">
                {item.modules.map((module, i) => (
                  <span
                    key={i}
                    className="px-2 py-1 bg-gray-50 border border-gray-100 text-[9px] font-bold text-gray-500 uppercase rounded"
                  >
                    {module}
                  </span>
                ))}
